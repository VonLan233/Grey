...
- [x] 代码路径级语义视图的上下文联动（Agent 自动接收 `lsp_*` 工具结果并按 `tool/path` 注入会话语义摘要）。
- [x] 诊断到会话的实时注入策略（`Agent` 将 `lsp_diagnostics` 等工具结果转为语义视图消息，写入下一轮上下文）。
- [x] LSP 工具输出的缓存/去噪与 token 预算融合（上下文侧按 `tool/path` 去重，计入 `ContextAudit.tool_outputs_deduplicated`）。
